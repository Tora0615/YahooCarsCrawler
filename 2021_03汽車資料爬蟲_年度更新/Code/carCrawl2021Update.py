# -*- coding: utf-8 -*-
"""
Created on Tue Mar  2 18:53:38 2021

@author: Arthur
"""

#2021/03/03 - 08:58pm 結束
#跑了8小時左右 (最後part : 5 小時 37 分 45 秒)
...
all_tag = ['品牌ID','品牌名稱','生產年份','型號名稱','型號ID','年份型號合併','版本ID','版本名稱','新車售價','動力型式','引擎型式','排氣量','最大馬力','最大扭力','驅動型式','變速系統','前輪懸吊','後輪懸吊','煞車型式','輪胎尺碼(前)','輪胎尺碼(後)','車身型式','車門數','座位數','車長','車寬','車高','車重','軸距','市區油耗','高速油耗','平均油耗','牌照稅','燃料費']
for j in range(len(all_tag)):
     sheet1.write(0,j,all_tag[j]) #年份
    ...
    for k in range(4):  #四台車
        ...
        while True:
            try:
                ...
                workbook.save('allcar_all_info.xls')  #若要一次跑完用這個；若程式曾中斷，用上面的，這個要註解掉
                break
            except:
